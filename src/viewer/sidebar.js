...
export class Sidebar {
	...
	initToolbar() {
		...
		{ // SHOW / HIDE Measurements
			...
			let title = `<span data-i18n="measurements.show_hide_labels">Show/Hide labels</span>`;
			elShow.selectgroup({ title: title });
	...
	initScene() {
...

		{
			...
			elExport.append(`
				<span data-i18n="scene.export">Export:</span> <br>
		...
		let pcID = tree.jstree('create_node', "#", { "text": "<b data-i18n='tb.point_clouds'>Point Clouds</b>", "id": "pointclouds" }, "last", false, false);
		...
		let otherID = tree.jstree('create_node', "#", { "text": "<b data-i18n='tb.other_opt'>Other</b>", "id": "other" }, "last", false, false);
		let vectorsID = tree.jstree('create_node', "#", { "text": "<b data-i18n='tb.vectors_opt'>Vectors</b>", "id": "vectors" }, "last", false, false);
		let imagesID = tree.jstree('create_node', "#", { "text": "<b data-i18n='tb.images_opt'> Images</b>", "id": "images" }, "last", false, false);
		...
		let onOrientedImagesAdded = (e) => {
			...
			const node = createNode(imagesID, "<span data-i18n='tb.images_opt'>images</span>", imagesIcon, images);
		...
		let onImages360Added = (e) => {
			...
			const imagesIcon = `${Potree.resourcePath}/icons/picture.svg`;
			const node = createNode(imagesID, "<span data-i18n='tb.images360_opt'>360° images</span>", imagesIcon, images);
		...
		{
			createNode(otherID, "<span data-i18n='scene.camera'>Camera</span>", null, new THREE.Camera());
		}
	...
	initClippingTool() {
...

		{
			...
			let title = `<span data-i18n="clipping.clip_task">Clip Task</span>`;
			elClipTask.selectgroup({ title: title });
...

		{
			...
			let title = `<span data-i18n="clipping.clip_method">Clip Method</span>`;
			elClipMethod.selectgroup({ title: title });
	...
	initClassificationList() {
			...
			const element = $(`
				<li>
					<label style="whitespace: nowrap">
						...
						<span data-i18n="filters.show_hide_all">show/hide all</span>
					</label>
				</li>
			`);
		...
		const addInvertButton = () => {
			const element = $(`
				<li>
					<input type="button" value="invert" data-i18n="[value]filters.invert" />
				</li>
			`);
	...
	initSettings() {
...

		{
			...
			let title = `<span data-i18n="appearance.splat_quality">Splat Quality</span>`;
			elSplatQuality.selectgroup({ title: title });
